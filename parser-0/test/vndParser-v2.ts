import { ParseResult, ParsedScene, SceneFile, InitScript, SceneConfig, Hotspot, HotspotCommand, InitCommand, TooltipInfo } from './types.ts';
...
export class VNDSequentialParser {
  ...
  private tryParseTooltip(offset: number, limit: number): { tooltip: TooltipInfo, nextOffset: number } | null {
      ...
      // Type généralement petit (0-10)
      if (type > 20) return null;

      // Coordonnées raisonnables pour écran 640x480 (avec marge)
      if (x1 > 800 || y1 > 600 || x2 > 800 || y2 > 600) return null;

      // Rectangle VALIDE: x2 >= x1, y2 >= y1 (STRICT)
      if (x2 < x1) return null;
      if (y2 < y1) return null;

      // Rectangle doit avoir une taille minimale raisonnable
      const width = x2 - x1;
      const height = y2 - y1;
      if (width < 10 || height < 10) return null;

      // Flag généralement 0 ou très petit
      if (flag > 10) return null;

      // Longueur de chaîne raisonnable (tooltip = texte court)
      ...
      for (let i = 0; i < strLen; i++) {
          ...
          // Caractères de contrôle (sauf null en fin)
          if (c < 32 && c !== 0) {
              controlChars++;
          }
          // Caractères imprimables Windows-1252 (32-126 + accents 128-255)
          if ((c >= 32 && c <= 126) || (c >= 128 && c <= 255)) {
              validChars++;
          }
      }

      // AUCUN caractère de contrôle accepté (sauf null)
      if (controlChars > 0) return null;

      // Au moins 90% de caractères valides
      ...
      // Le texte doit avoir du contenu
  ...
  private parseSceneBlock(id: number, start: number, limit: number): ParsedScene {
      ...
      // NOUVEAU: Scanner toute la zone après les hotspots standards pour les tooltips
      // (ils peuvent être entre les hotspots normaux et la fin de scène)
      ...
      if (detectedTooltips.length > 0) {
          this.log(`  [INFO] ${detectedTooltips.length} tooltip(s) trouvé(s) dans la zone 0x${tooltipScanStart.toString(16)}-0x${limit.toString(16)}`);
      }

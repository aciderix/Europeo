...
import { Image, Box, MousePointer2, Music, Type, Code, Move, MonitorPlay, MousePointerClick, MessageSquare, Video, ScrollText, AlertTriangle, CheckCircle2, FileWarning, Microscope, FileImage, Trophy, LifeBuoy, Ghost, HelpCircle, Film, Target, Sparkles, Database } from 'lucide-react';
                  ...
                  <div key={i} className={`bg-slate-900/80 border border-slate-700 rounded p-3 transition-colors ${hs.isRecovered ? 'border-indigo-900/30' : ''}`}>
                    <div className="flex justify-between items-center mb-2 pb-2 border-b border-slate-800">
                       ...
                       <span className="text-xs text-slate-500 font-mono flex gap-2">
                         ...
                         <span className="bg-slate-800 px-1 rounded text-slate-400">Cmds: {hs.commands.length}</span>
                       </span>
                    </div>
                        ...
                        <div className="relative h-full min-h-[60px] bg-slate-950 rounded border border-slate-800 p-2 flex items-center justify-center">
                             {hs.geometry.points.length > 0 ? (
                                <div className="w-full text-center">
                                    ...
                                    <svg viewBox="0 0 640 480" className="w-full h-24 bg-slate-900 border border-slate-800 rounded">
                                        <polygon 
